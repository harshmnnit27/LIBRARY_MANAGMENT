...
const MyBorrowedBooks = () => {
  ...
  return (
    <>
      <main className="relative flex-1 p-6 pt-28 bg-[#F8FAFC] text-[#001F3F]">
        <Header />
        ...
        {booksToDisplay && booksToDisplay.length > 0 ? (
            ...
            <table className="min-w-full border-collapse">
              ...
              <tbody>
                {booksToDisplay.map((book, index) => (
                  <tr
                    key={index}
                    className={(index + 1) % 2 === 0 ? "bg-[#f1f5f9]" : ""}
                  >
                    <td className="px-4 py-2">{index + 1}</td>
                    <td className="px-4 py-2">{book.bookTitle}</td>
                    <td className="px-4 py-2">{formatDate(book.borrowedDate)}</td>
          ...
          <h3 className="text-3xl mt-5 font-medium text-center">No returned books found!</h3>
        ) : (
          <h3 className="text-3xl mt-5 font-medium text-center">No non-returned books found!</h3>
        )}
      </main>
