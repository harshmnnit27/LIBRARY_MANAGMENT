...
// import { BookA, NotebookPen, NotebookPenIcon } from "lucide-react";
    ...
    <main className="relative flex-1 p-6 pt-28 text-[#001F3F]">
      ...
      <header className="flex flex-col gap-3 md:flex-row md:justify-between md:items-center">
        ...
        <div className="flex flex-col lg:flex-row space-y-4 lg:space-y-0 lg:space-x-4 ">
          {
            isAuthenticated && user?.role === "Admin" && (
              <button className="relative pl-14 w-full sm:w-52 flex gap-4 justify-center items-center py-2 px-4 bg-[#001F3F] text-white rounded-md hover:bg-[#003366]" onClick={() => dispatch(toggleAddBookPopup())}>
                <span className="bg-white flex justify-center items-center overflow-hidden rounded-full text-black w-[25px] h-[25px] text-[27px] absolute left-5">+</span>
                Add Book
              </button>
            )
      ...
      {searchedBooks.map((book, index) => (
        <tr
          ...
          className={`transition duration-200 ${
            (index + 1) % 2 === 0 ? "bg-[#f2f8ff]" : "bg-white"
          } hover:bg-[#f0f8ff]`}
        >
          <td className="px-4 py-2">{index + 1}</td>
          <td className="px-4 py-2">{book.title}</td>
